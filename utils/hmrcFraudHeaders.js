...
export function buildFraudHeaders(req, user_id) {
  ...
  // ✅ ONE MFA method (can add more later separated by comma)
  const mfa = `type=OTHER&timestamp=${timestamp}&uniqueReference=${deviceId}`;
  ...
  return {
    ...
    // ✅ Correct screen format (single screen)
    "Gov-Client-Screens":
    ...
    // ✅ CRITICAL: encode entire MFA string ONCE
    "Gov-Client-Multi-Factor": encodeURIComponent(mfa),
